# Peter Laurin
# Feb 24, 2025
# Adapted from https://github.com/zjshi/snv_analysis_almeida2019.git to Python3
...
def read_catalog(fpath):
	...
	#	n_col = len(next(fh).rstrip().split('\t'))

	#np.loadtxt(open(fpath, "r"), delimiter="\t", dtype=np.uint8, skiprows=1, usecols = range(1,n_col))
...
def read_coords(coords_paths):
	...
	for coords_path in coords_paths:
		ref_tag = coords_path.split('/')[-1].split('-')[0]
		genome_tag = coords_path.split('/')[-1].split('-')[-1].split('.coords')[0]
